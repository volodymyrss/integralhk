...
def try_all(key):
    def decr(f):
        def nf(*a,**b):
            ...
            print "exceptions:",exceptions.items()
                
            for k1 in exceptions.keys():
                for k2 in exceptions.keys():
                    if k1==k2: continue
                    ...
                    try:
                        ...
                        print "no...",e
                        continue
            exceptions_not_none=dict([(k,v) for k,v in exceptions.items() if v is not None])
            if len(exceptions_not_none.keys())==1:
                e=exceptions_not_none.values()[0]
